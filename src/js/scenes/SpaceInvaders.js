...
export default class SpaceInvaders extends Phaser.Scene {
    ...
    initEnemys() {
      for (let i = 0; i < enemyInfo.count.col; i++) {
          for (let j = 0; j < enemyInfo.count.row; j++) {
              let enemyX = (i * (enemyInfo.width + enemyInfo.padding)) + enemyInfo.offset.left;
              let enemyY = (j * (enemyInfo.height + enemyInfo.padding)) + enemyInfo.offset.top;
    ...
    manageGreenLaser(greenLaser) {
      ...
      let i = setInterval(function () {
          ...
          for (let step = 0; step < bigInvaders.length; step++) {
              let bigInvader = bigInvaders[step];
    ...
    manageRedLaser(redLaser, enemy) {
      ...
      let angle = Phaser.Math.Angle.BetweenPoints(enemy, this.player);
